...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        # x: (B, C, T) = (256, 1, 32)
        x_t = x      # no transpose needed because C = 1
        x_t = x_t.squeeze(1)              # (B, T)
        scores = self.attn(x_t)           # (B, T)
        weights = self.softmax(scores)    # (B, T)
        attended = x_t * weights          # (B, T)
        out = attended.unsqueeze(1)       # (B, 1, T)
        return x + out                    # residual connection


class bens_transpose_1024(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.en_attention = AttentionBlock(seq_len=self.dense_1_in)
        ...
        self.de_layer1 = nn.Sequential(
            nn.ConvTranspose1d(
            in_channels=kwargs["input_shape"], out_channels=kwargs["input_shape"],
            kernel_size=self.kernel_1, stride=2, padding = self.kernel_1//2, output_padding
            = 1),
            nn.BatchNorm1d(num_features=kwargs["input_shape"]),
            nn.LeakyReLU(negative_slope = self.negative_slope)
        )
        self.de_layer2 = nn.Sequential(
            nn.ConvTranspose1d(
            in_channels=kwargs["input_shape"], out_channels=kwargs["input_shape"],
            kernel_size=self.kernel_2, stride=2, padding = self.kernel_2//2, output_padding
            = 1),
            nn.BatchNorm1d(num_features=kwargs["input_shape"]),
            nn.LeakyReLU(negative_slope = self.negative_slope)
        )
        self.de_layer3 = nn.Sequential(
            nn.ConvTranspose1d(
            in_channels=kwargs["input_shape"], out_channels=kwargs["input_shape"],
            kernel_size=self.kernel_3, stride=2, padding = self.kernel_3//2, output_padding
            = 1),
            nn.BatchNorm1d(num_features=kwargs["input_shape"]),
            nn.LeakyReLU(negative_slope = self.negative_slope)
        )
        self.de_layer4 = nn.Sequential(
            nn.ConvTranspose1d(
            in_channels=kwargs["input_shape"], out_channels=kwargs["input_shape"],
            kernel_size=self.kernel_4, stride=2, padding = self.kernel_4//2, output_padding
            = 1),
            nn.BatchNorm1d(num_features=kwargs["input_shape"]),
            nn.LeakyReLU(negative_slope = self.negative_slope)
        )
        self.de_layer5 = nn.Sequential(
            nn.ConvTranspose1d(
            in_channels=kwargs["input_shape"], out_channels=kwargs["input_shape"],
            kernel_size=self.kernel_5, stride=2, padding = self.kernel_5//2, output_padding
            = 1),
            nn.BatchNorm1d(num_features=kwargs["input_shape"]),
            nn.LeakyReLU(negative_slope = self.negative_slope)
        )
        ...
        self.de_attention = AttentionBlock(seq_len=self.dense_1_out)
...
class encode(bens_transpose_1024):
    ...
    def forward(self,features):
        ...
        activation = self.en_attention(activation)
...
class decode(bens_transpose_1024):
    def forward(self, features):
